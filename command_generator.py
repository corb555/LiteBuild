...
class CommandGenerator:
    ...
    def generate_for_node(
        ...
        all_resolved_inputs = self._resolve_all_inputs(
            node_name, node_data, context, resolved_outputs, self.target_config
        )
        positional_filenames_templates = node_data.get("POSITIONAL_FILENAMES", [])

        # --- Validation for Unused Structures ---
        ...
        if final_params and "{PARAMETERS}" not in command_template:
            error_msg = (f"❌ Configuration Error in WORKFLOW Step '{node_name}':\n"
                         ...
                         f"template "
                         "is missing the '{PARAMETERS}' placeholder.\n\n"
                         f"💡 The following parameters would be ignored: "
                         f"{list(final_params.keys())}\n\n"
                         f"   To fix this, add {{PARAMETERS}} to  'WORKFLOW / {node_name} / RULE "
                         f"/ COMMAND: '")
            raise ValueError(error_msg)

        if positional_filenames_templates and "{POSITIONAL_FILENAMES}" not in command_template:
            error_msg = (f"❌ Configuration Error in WORKFLOW Step '{node_name}':\n"
                         f"'POSITIONAL_FILENAMES' were defined, but the COMMAND  "
                         "is missing the '{POSITIONAL_FILENAMES}' placeholder.\n\n"
                         f"💡 To fix this, add {{POSITIONAL_FILENAMES}} to  'WORKFLOW / "
                         f"{node_name} / RULE / COMMAND: '")
            raise ValueError(error_msg)

        has_inputs = node_data.get("INPUTS")
        uses_inputs_in_command = any(
            p in command_template for p in ["{INPUTS}", "{INPUTS["]
        )
        uses_inputs_in_positional = any(
            "{INPUTS" in tmpl for tmpl in positional_filenames_templates
        )

        if has_inputs and not (uses_inputs_in_command or uses_inputs_in_positional):
            error_msg = (f"⚠️  Configuration Warning in WORKFLOW Step '{node_name}':\n"
                         f"The 'INPUTS' key was defined, but no input placeholders like '{{"
                         f"INPUTS[0]}}' were found in the COMMAND or POSITIONAL_FILENAMES.\n\n"
                         f"💡 The following inputs are unused: {has_inputs}")
            raise ValueError(error_msg)
    ...
    def _merge_parameters(self, node_name: str, node_data: dict, context: dict) -> dict:
        ...
        target_params = self.target_config.get("PARAMETERS", {}).get(rule_name, {})
        workflow_params = node_data.get("PARAMETERS", {})

        merged = {**general_params, **target_params, **workflow_params}
    ...
    def _resolve_all_inputs(
        ...
        target_files = target_config.get("TARGET_FILES")
        ...
        for tmpl in input_templates:
            ...
            if '{TARGET_FILES}' in tmpl:
                if not target_files:
                    raise ValueError(
                        f"Error in '{node_name}': Uses {{TARGET_FILES}} but target is missing a "
                        f"'TARGET_FILES' list."
                    )
                for f in target_files:
                    file_context = {**context, 'TARGET_FILES': f}
                    all_inputs.append(self._deep_template(node_name, tmpl, file_context))
                continue

            all_inputs.append(self._deep_template(node_name, tmpl, context))
        return all_inputs
    ...
    def _build_command_string(
        ...
        dash_style = rule_data.get("DASH", "-")
        unquoted_params = rule_data.get("UNQUOTED_PARAMS", [])
        params_str = self._format_shell_params(params, dash_style, unquoted_params)
        positional_filenames_str = " ".join([shlex.quote(p) for p in positional_filenames])
        # Create the space-separated string for the {INPUTS} placeholder.
        inputs_str = " ".join([shlex.quote(p) for p in inputs])

        # Create a single, unified context for the final formatting step.
        template_context = {
        ...
        try:
            ...
            return final_template.format_map(self.SafeFormatter(template_context)).strip().replace(
                '  ', ' '
            )
        except Exception as e:
            raise RuntimeError(f"Error formatting command for '{node_name}': {e}") from e
    ...
    def _deep_template(self, node_name: str, data: Any, context: dict) -> Any:
        if isinstance(data, str):
            ...
            for _ in range(5):
                ...
                if templated_string == prev_string:
                    break
            try:
                ...
                raise ValueError(
                    f"Error in '{node_name}': Undefined variable {{{missing_key}}} in string: \""
                    f"{data}\""
                )
